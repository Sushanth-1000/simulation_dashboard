...
### D-12k · The fifth candidate against the slow drift, and the claim it broke

**The only entry in this log where the decision was to attack the project's own conclusion.**

| | |
|---|---|
| **Forced by** | Being asked *"can't you try detecting this?"* about a fault four measured refutations had closed the book on. The honest answer required checking **what each refutation actually tested** rather than citing the count. E-53 and E-105 both tested innovation **magnitude** — *"every step is well inside what the filter expects"* — and **neither tested whether the sequence is white**, which is the classical detector for exactly this failure and a different question with a different answer |
| **Alternatives** | (a) **Cite E-107 and stop** — four refutations and a stated root cause is a strong position, and it would have been wrong. (b) **Re-run one of the four** — repeats a result rather than adding one. (c) **Wait for redundancy (OD-15)** — the known answer, and it does not license the claim that *nothing else* works. (d) **Build the untried statistic and measure it** |
| **Why this one** | E-107 asserted that **no function of a single sensor chain** can separate a self-consistent slow lie. That is a universal claim resting on three instances, and a universal claim is exactly the kind this project's own conventions say must be attacked rather than accumulated. It broke: the signed residual carries **+0.242σ** against a clean **−0.003σ**, and a CUSUM separates **7.35x** (E-139, E-140) |
| **Gave up** | **Nothing is wired**, deliberately — no mechanism gets authority until it has run with none, and this one has three blockers before it could earn any: it ran against the **placeholder proposer** because no policy checkpoint is in the tree; the slack was chosen **knowing the fault's magnitude**; and the signed residual **reaches no archive**, only its norm (E-142) |
| **The near miss** | At the textbook slack `k = 0.5` the detector reports **1.21x** and would have been filed as a fifth refutation, strengthening a claim that is false. It was the *sweep* that found it — the same discipline E-94 used, applied because a single parameter value is not a measurement. **A detector is not refuted until its free parameter has been swept**, and that is the transferable lesson here |
| **The uncomfortable part** | E-107 is retracted in place rather than corrected, per D-15. The three measurements it rests on are all still exactly true; the **generalisation** drawn from them was not, and it had been carried into the register, the module docstrings and the case for Phase 7. A wrong conclusion built from correct measurements is the hardest kind to notice, because every citation checks out |
